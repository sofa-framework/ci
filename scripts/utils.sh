    ...
    if vm-is-windows; then
        ...
        if [ $msvc_year -le 2015 ]; then
            ...
            $COMSPEC //c "$vcvarsall && cd $build_dir_windows && $cmake_command $*"
				...
				echo "Calling: $cmake_command  $@ -DCMAKE_CXX_FLAGS=\"-ffp-contract=off\""  | tee -a "$build_dir/cmake-output.txt"
				cd $build_dir && $cmake_command "$@" -DCMAKE_CXX_FLAGS="-ffp-contract=off"  | tee -a "$build_dir/cmake-output.txt"
		else
